...
#setwd("C:/Users/liuwe/Dropbox (YSE)/YASSP/PROJECTS/DOE_Road to Removal/case_studies/new_england_forests/FVS_simulation/rFVS/FVSProjects/sensitivity_dist")
...
start_time <- Sys.time()
r <- mclapply(c(8,9), function(n) {    
  fvsSetCmdLine(mgmtid[n])
  results <- lapply(stdno,runFVS)
}, mc.cores=3)    
print(Sys.time() - start_time)

fvsSetCmdLine('--keywordfile=./keyfile/SLLD.key')
start_time <- Sys.time()
results <- lapply(stdno,runFVS)
print(Sys.time() - start_time)
...
clusterExport(cl, c("stdno", "runFVS","mgmtid",'.FVSLOADEDLIBRARY'), 
              envir=environment())
start_time <- Sys.time()
r <- parLapply(cl,c(8,9),function(n) {
  fvsSetCmdLine(mgmtid[n])
  results <- lapply(stdno,runFVS)
})
print(Sys.time() - start_time)